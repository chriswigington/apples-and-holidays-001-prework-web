...
def all_supplies_in_holidays(holiday_hash)
  ...
  # etc.
  holiday_hash.each do |key, holiday_list|
    season = key.to_s.capitalize
    puts "#{season}:"
    holiday_list.each do |holiday, supplies|
      holiday_string = holiday.to_s.split("_")
      holiday_string.each do |x|
        x.capitalize!
      end
      holiday_string = holiday_string.join(" ")
      #binding.pry
      full_string = "  #{holiday_string}: "
      count = 0
      while count < supplies.length
        supply = supplies[count]
        if count != supplies.length - 1
          full_string << "#{supply}, "
        else
          full_string << supply
        end
